import Link from "next/link";
import { ArrowDownLeft, ArrowUpRight, CalendarDays, CreditCard, Tag } from "lucide-react";

import { appRoutes } from "@/lib/constants/routes";
import { formatCurrencyFromCents } from "@/lib/utils/currency";
import { formatEntryDate } from "@/lib/utils/date";

type DashboardRecentEntriesProps = {
  entries: Array<{
    id: string;
    type: "income" | "expense";
    amountCents: number;
    description: string;
    entryDate: string;
    categoryName: string;
    categoryColor: string;
  }>;
};

export function DashboardRecentEntries({ entries }: DashboardRecentEntriesProps) {
  return (
    <section className="rounded-[2rem] border border-white/12 bg-white/6 p-6 backdrop-blur-xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">Últimos lançamentos</h2>
          <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">
            Os 5 lançamentos mais recentes continuam visíveis aqui para consulta rápida.
          </p>
        </div>

        <Link
          href={appRoutes.entries}
          className="inline-flex shrink-0 items-center rounded-full border border-white/12 bg-white/8 px-4 py-2 text-sm font-medium text-[var(--text-primary)] transition hover:bg-white/12"
        >
          Ver todos
        </Link>
      </div>

      {!entries.length ? (
        <div className="mt-5 rounded-[1.5rem] border border-dashed border-white/12 bg-black/10 p-6 text-center">
          <p className="text-base font-medium text-[var(--text-primary)]">Nenhum lançamento recente ainda</p>
          <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">
            Assim que você registrar receitas ou despesas, os últimos itens aparecerão aqui.
          </p>
        </div>
      ) : (
        <div className="mt-5 space-y-3">
          {entries.map((entry) => {
            const isIncome = entry.type === "income";

            return (
              <Link
                key={entry.id}
                href={`${appRoutes.entries}/${entry.id}/edit`}
                className="block rounded-[1.5rem] border border-white/10 bg-black/10 p-4 transition hover:border-white/16 hover:bg-white/8"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-3">
                      <span
                        className="flex size-10 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/8"
                        style={{ color: entry.categoryColor }}
                      >
                        {isIncome ? <ArrowUpRight className="size-4" /> : <ArrowDownLeft className="size-4" />}
                      </span>

                      <div className="min-w-0">
                        <p className="truncate font-medium text-[var(--text-primary)]">{entry.description}</p>
                        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[var(--text-muted)]">
                          <span className="inline-flex items-center gap-1">
                            <Tag className="size-3" />
                            {entry.categoryName}
                          </span>
                          <span className="inline-flex items-center gap-1">
                            <CalendarDays className="size-3" />
                            {formatEntryDate(entry.entryDate)}
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div className="text-right">
                    <p
                      className={`font-mono text-base font-semibold ${
                        isIncome ? "text-emerald-300" : "text-rose-300"
                      }`}
                    >
                      {isIncome ? "+" : "-"}
                      {formatCurrencyFromCents(entry.amountCents)}
                    </p>
                    <p className="mt-1 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
                      {isIncome ? "Receita" : "Despesa"}
                    </p>
                  </div>
                </div>
              </Link>
            );
          })}
        </div>
      )}

      <div className="mt-5 flex items-center gap-2 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
        <CreditCard className="size-3.5" />
        Sempre mostrando os lançamentos mais recentes, independente do mês filtrado.
      </div>
    </section>
  );
}
